...
<head>
    ...
    <title>Admin - Approve Report</title>
...
if (isset($_GET['report_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($issueStatus == 1) {
            // Update the issue status to "รอการแก้ไข" (Waiting for Repair) and use yellow color
            $updateSql = "UPDATE reports SET issue_status = 0 WHERE report_id = $reportId";
            if ($connection->query($updateSql) === TRUE) {
                // Display a success message with the new status using SweetAlert
                echo "<script>
                        Swal.fire({
                            title: 'Success!',
                            text: 'The report is now waiting for repair.',
                            icon: 'success',
                ...
                echo "<script>
                        Swal.fire({
                            title: 'Error!',
                            text: 'There was an error updating the report status.',
                            icon: 'error',
                            confirmButtonText: 'OK'
                        });
                    </script>";
            ...
            // Display a message using SweetAlert if the report is already resolved or in another state
            echo "<script>
                    Swal.fire({
                        title: 'Warning!',
                        text: 'This report has already been resolved or is in another state.',
                        icon: 'warning',
                        confirmButtonText: 'OK'
                    }).then((result) => {
                        if (result.isConfirmed) {
